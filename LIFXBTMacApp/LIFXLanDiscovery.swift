...
//    private func sendPacket(to endpoint: NWEndpoint, packet: Data) {
//        let params = NWParameters.udp
//        // Do NOT bind to listenPort — the listener already owns it.
//        // Outbound sends use an ephemeral port; devices reply to the listener
//        // via the broadcast/source address regardless of send port.
//        params.includePeerToPeer = false
//        params.prohibitedInterfaceTypes = [.loopback]
//
//        let conn = NWConnection(to: endpoint, using: params)
...
//            params.acceptLocalOnly = false
//
//            // Must listen on 56700 — LIFX devices always reply to port 56700
//            let listener = try NWListener(using: params, on: NWEndpoint.Port(rawValue: 56700)!)
//            self.listener = listener
//
//            listener.stateUpdateHandler = { [weak self] state in
//                guard let self else { return }
...
//        )
//
//        // Use BSD socket for broadcast — NWConnection blocks sendmsg to broadcast addresses
//        let address = getBroadcastAddress() ?? "10.255.255.255"
//        print("📡 [LIFX Discovery] Broadcasting GetService to \(address):56700")
//        sendBroadcastBSD(to: address, packet: packet)
//    }
//    
//    // IMPROVEMENT: Calculate subnet broadcast address
//    private func getBroadcastAddress() -> String? {
...
//    }
//
//    /// Sends a UDP broadcast using a raw BSD socket, bypassing NWConnection sandbox restrictions.
//    private func sendBroadcastBSD(to address: String, packet: Data) {
//        let sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)
//        guard sock >= 0 else {
//            print("❌ [LIFX Discovery] BSD socket() failed: \(errno)")
//            return
//        }
//        defer { close(sock) }
//
...
//
//        // Bind to port 56700 so devices send StateService replies back to us
//        var bindAddr = sockaddr_in()
//        bindAddr.sin_family = sa_family_t(AF_INET)
//        bindAddr.sin_port = UInt16(56700).bigEndian
...
//        }
//        if bindResult < 0 {
//            print("⚠️ [LIFX Discovery] BSD bind to 56700 failed (\(errno)) — replies may not arrive")
//        }
//
...
//        }
//        if sent < 0 {
//            print("❌ [LIFX Discovery] BSD sendto failed: \(errno)")
//        } else {
//            print("📡 [LIFX Discovery] BSD broadcast sent \(sent) bytes to \(address):56700")
//        }
//    }
//
//        private func sendPacket(to endpoint: NWEndpoint, packet: Data) {
...
final class LIFXLanDiscovery {
    ...
    private func sendGetLabel(to ip: String, target: Data) {
        ...
        print("🏷 [LIFX Discovery] Requesting label from \(ip)")
        sendUnicastBSD(to: ip, packet: packet)
    }
    ...
    private func sendBroadcastBSD(to address: String, packet: Data) {
        ...
        // Bind failure (errno 48 = already in use) is expected on retries — NWListener owns the port.
        // The send still succeeds; devices are already discovered via the listener.
        _ = bindResult
    ...
    /// Sends a UDP unicast using a raw BSD socket — NWConnection can't share port 56700 with NWListener.
    private func sendUnicastBSD(to ip: String, packet: Data) {
        let sock = socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP)
        guard sock >= 0 else { print("❌ [LIFX Discovery] BSD socket() failed: \(errno)"); return }
        defer { close(sock) }
        ...
        dst.sin_port = UInt16(56700).bigEndian
        dst.sin_addr.s_addr = inet_addr(ip)

        let sent = packet.withUnsafeBytes { ptr in
            withUnsafePointer(to: &dst) { dstPtr in
                dstPtr.withMemoryRebound(to: sockaddr.self, capacity: 1) {
                    sendto(sock, ptr.baseAddress, packet.count, 0, $0, socklen_t(MemoryLayout<sockaddr_in>.size))
                }
            }
        }
        if sent < 0 { print("❌ [LIFX Discovery] BSD unicast sendto \(ip) failed: \(errno)") }
    }
